...
const LASTLINK_URL = "https://lastlink.com/p/{{LASTLINK_PRODUTO_LOW}}/checkout-payment/";
const OFFER_PRICE = 397;
    ...
    try {
      ...
      if (typeof window !== "undefined" && (window as any).fbq) {
        (window as any).fbq(
          ...
          "Lead",
          {
            content_name: "Funil ConsultorIA",
            content_category: "high_ticket_lp",
            value: OFFER_PRICE,
            currency: "BRL",
          },
          { eventID: eventId }
      ...
      // 3. Pixel InitiateCheckout (intenção forte de compra pra Meta)
      if (typeof window !== "undefined" && (window as any).fbq) {
      ...
      //    Cookie permite que o /obrigado em qualquer subdomínio leia (ex: form na LP →
      //    Lastlink → redirect pra /obrigado em outro subdomínio compartilhado).
      const identityPayload = JSON.stringify({
        email: emailTrim,
        event_id: eventId,
        funnel_source: "funil-consultor-ia",
      });
      try {
      ...
      // 5. Redireciona pro Lastlink com trk = eventId pra atribuição completa Pixel ↔ CAPI ↔ Lastlink webhook
      ...
      <DialogContent className="max-w-lg bg-card border-border">
        ...
        <div className="flex items-start gap-3 p-4 rounded-lg border border-red-500/40 bg-red-950/30">
          ...
          <p className="text-sm text-red-100 leading-relaxed">
            ...
            valor promocional de R${OFFER_PRICE}. Garanta a sua agora antes que o preço suba!
          </p>
        </div>
