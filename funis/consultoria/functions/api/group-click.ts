...
//
// Idempotente: se o lead clicar 2x, o GHL workflow re-aplica a mesma stage
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const funnelSource = (body.funnel_source || "funil-consultor-ia").trim();
  ...
  // Encaminha pro webhook GHL (move opportunity → stage "Entrou no Grupo do Webinar")
  ...
  // Persiste o evento no D1 pra auditoria/debug (tabela quiz_leads tem coluna ghl_response_body
  // que reusamos pra log; se quiser tabela dedicada no futuro, criar group_clicks).
