...
interface LeadPayload {
  ...
  answers?: unknown;
  /** Optional: which funnel/page the lead came from. Defaults to "lp-negocio-ia". */
  source?: string;
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const sourceTag = (body.source && body.source.trim()) || "webinar-ao-vivo";
  const sourceMap: Record<string, { name: string; category: string; url: string }> = {
    "webinar-ao-vivo": {
      name: "Webinar ConsultorIA Ao Vivo",
      category: "webinar_registration",
      url: "https://{{WEBINAR_DOMAIN}}/",
    },
    "funil-consultor-ia": {
      name: "Funil ConsultorIA",
      category: "high_ticket_lp",
      url: "https://SEU-DOMINIO.com/",
    },
    "lp-negocio-ia": {
      name: "LP Negocio.IA",
      category: "low_ticket_lp",
      url: "https://SEU-DOMINIO.com/",
    },
    "quiz-negocio-ia": {
      name: "Quiz Negocio.IA",
      category: "quiz_funnel",
      url: "https://SEU-DOMINIO.com/",
    },
  };
  const meta = sourceMap[sourceTag] || sourceMap["funil-consultor-ia"];
  const contentName = meta.name;
  const contentCategory = meta.category;
  ...
  const trackerPromise = fetch(trackerUrl, {
    ...
    body: JSON.stringify({
      ...
      event_source_url: referrer || sourceUrl,
