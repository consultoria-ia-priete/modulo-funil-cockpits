...
import { CheckCircle2, ChevronDown, CalendarClock, AlertTriangle, ArrowRight, Loader2, Mail, MessageCircle } from "lucide-react";

const WHATSAPP_GROUP_URL = "https://SEU-DOMINIO.com/grupos";
const FUNNEL_SOURCE = "webinar-ao-vivo";
...
const Obrigado = () => {
  ...
  const handleGroupClick = async (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    try {
      ...
      // Server-side: move o stage do opportunity pra "Entrou no Grupo do Webinar"
    ...
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-primary/30">
      ...
      <main className="relative z-10 container mx-auto px-4 py-10 md:py-16 min-h-screen flex flex-col items-center justify-center">
        <div className="w-full max-w-2xl mx-auto text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          {/* lembrete do horário */}
          <div className="inline-flex items-center gap-3 px-5 py-3 mb-10 rounded-xl bg-card border border-primary/30">
            <CalendarClock className="w-5 h-5 text-primary" />
            <span className="font-bold">Hoje às 14:30</span>
            <span className="text-muted-foreground text-sm">(horário de Brasília)</span>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-10 text-left">
            <div className="flex items-start gap-3 p-4 rounded-xl bg-card border border-border">
              ...
              <p className="text-sm text-muted-foreground">
                O link da sala chega no seu <span className="text-foreground font-bold">WhatsApp</span> com um lembrete antes de começar.
              </p>
            </div>
            <div className="flex items-start gap-3 p-4 rounded-xl bg-card border border-border">
              <Mail className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" />
              <p className="text-sm text-muted-foreground">
                Também enviamos por <span className="text-foreground font-bold">e-mail</span>. Confere a caixa de entrada e o spam.
              </p>
            </div>
          </div>
          ...
          <div className="bg-card border-2 border-primary/50 rounded-2xl p-6 md:p-8 text-left animate-card-glow">
            ...
            <p className="text-muted-foreground mb-6 leading-relaxed">
              É no grupo que o{" "}
              <span className="text-foreground font-bold">link da aula ao vivo, o lembrete e os avisos</span>{" "}
              são enviados. Quem entra no grupo é o primeiro a saber quando a sala abre.
            </p>
            ...
            <div className="flex items-start gap-3 p-4 mb-6 rounded-lg border border-red-500/30 bg-red-950/30">
              ...
              <p className="text-sm md:text-base text-red-200">
                Quem não entra no grupo corre o risco de <span className="font-bold underline decoration-red-400">perder o link</span> e ficar de fora da aula de hoje.
              </p>
            </div>
            ...
            <p className="text-xs text-center text-muted-foreground mt-4">
              O link da aula é enviado exclusivamente no grupo e por e-mail.
            </p>
          </div>
        </div>
      </main>
