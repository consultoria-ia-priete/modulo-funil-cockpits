...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const sourceTag = (body.source && body.source.trim()) || "lp-negocio-ia";
  const isQuiz = sourceTag === "quiz-negocio-ia";
  const contentName = isQuiz ? "Quiz Negocio.IA" : "LP Negocio.IA";
  const contentCategory = isQuiz ? "quiz_funnel" : "low_ticket_lp";
  const sourceUrl = isQuiz
